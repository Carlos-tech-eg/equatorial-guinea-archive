...
// Initialize Firebase (singleton for Next.js). Analytics only in browser.
const app = !getApps().length ? initializeApp(firebaseConfig) : getApp();
const analytics = typeof window !== "undefined" ? getAnalytics(app) : null;
const auth = getAuth(app);
const db = getFirestore(app);
// Bind explicitly to the default bucket so uploads/refs match Console (appspot vs firebasestorage.app aliases).
const bucketId = firebaseConfig.storageBucket ?? "";
const storage = bucketId
